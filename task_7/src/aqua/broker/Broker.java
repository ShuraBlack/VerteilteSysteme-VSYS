...
public class Broker {
    ...
    private static final int LEASING_DURATION = 30;
    ...
    public Broker() {
        ...
        this.endpoint = new SecureEndpointAsymmetric(4711);
        this.connections = new ClientCollection<>();
        this.lock = new ReentrantReadWriteLock();
        this.timer = new Timer();

        final long interval = LEASING_DURATION * 1000L;
        this.timer.scheduleAtFixedRate(new TimerTask() {
            @Override
            public void run() {
                lock.readLock().lock();
                List<InetSocketAddress> expired = connections.getLeasingEnd();
                lock.readLock().unlock();
                expired.forEach(socket -> deregister(socket));
            }
        }, interval, interval);
    }
    ...
    public void broker() {
        ...
        while (!stopRequested) {
            ...
            if (msg != null && msg.getPayload() instanceof PoisonPill) {
                logger.info("Server got poisend and will shutdown soon!");
                break;
            }
    ...
    private void register(InetSocketAddress sender) {
        ...
        if (idx != -1) {
            final String id = this.connections.getClientID(idx);
            this.connections.updateRegistration(idx, LEASING_DURATION);
            endpoint.send(sender,new RegisterResponse(LEASING_DURATION, id));
            logger.log(Level.INFO, "Updated registration leasing for {0}", id);
            this.lock.writeLock().unlock();
            return;
        }
        final String id = "tank_" + (counterID++);
        this.connections.add(id,sender, LocalDateTime.now().plusSeconds(LEASING_DURATION));
        this.nameService.add(id, sender);
        endpoint.send(sender,new RegisterResponse(LEASING_DURATION, id));
        logger.log(Level.INFO, "New Client registered. ClientID: {0} - Activ Clients: ", this.connections.size());
        ...
        logger.log(Level.INFO,"Neighbors informed about new client");
    ...
    private void deregister(InetSocketAddress sender) {
        ...
        logger.log(Level.INFO,"Client deregistered and neighbors informed");
        this.lock.writeLock().unlock();
    }
    ...
    private void handoffFish(InetSocketAddress sender, HandoffRequest payload) {
        ...
        if (idx == -1) {
            logger.log(Level.FINER,"Client tried to handoff without register!");
            return;
        }
        // Rechts(1), Links(-1)
        if (dir.getVector() == 1) {
        ...
        logger.log(Level.FINER,"Hand off: {0}", payload.getFish().getId());
